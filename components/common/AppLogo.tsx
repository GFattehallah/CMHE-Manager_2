...
export const AppLogo: React.FC<AppLogoProps> = ({ className = "", size = 48, showText = false }) => {
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>   
     ...
     // >
          <div 
  className="w-12 h-12 md:w-16 md:h-16 lg:w-20 lg:h-20 flex items-center justify-center bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden shrink-0 transition-transform hover:scale-105 max-w-[20vw]"
>
       {!imageError ? (
       <img 
        src={resolvedUrl} 
